...
- ⏳ classes — only Barbarian fully populated; other 11 classes carry L1 features only. Transform script pending — Open5e v2's `/classes/` endpoint has structured features and `data_for_class_table` for progression columns, so the import is well-scoped (24 entries across both editions).
