...
**Creation-time stamps on `base_stats` go stale** — a handful of `Dnd5eStats` fields are written **once, by the character builder**, from the *starting* class and never rewritten by level-up or multiclassing. `spellcastingAbility` is the one that bit: a Rogue who multiclasses into Bard keeps the `null` their non-caster start wrote, so anything reading `stats.spellcastingAbility` directly sees a non-caster. That's why `getEffectiveSpellcastingAbility` resolves off `classResultsByKey` rather than trusting the stamp — and why SpellsTab / CombatTab / SkillsTab take the *resolved* ability, not the raw field. Symptom when this breaks: the Spells tab hides its whole spellcasting stats row (it was gated on the ability resolving), taking the **MANAGE** button with it, so the player has no way to add or swap spells at all. Prefer deriving from the resolved class results over reading a stamped stat; when a gate only needs "can this user edit spells?", don't hang it on a derived stat resolving.
...
**Split-screen page view** — Web-only. `useSplitPaneStore` (non-persisted Zustand) is a browser-style two-list tab model: `leftTabs` / `rightTabs` (arrays of `SplitTarget`), a per-side active index, `splitRatio` (0.2–0.8), and `focusedSide`. **Gotcha — the two hosts read the store differently.** The campaign route renders *both* sides from the store and hydrates them from the URL. The standalone world route (`[pageId].tsx`) renders its left pane from the URL `pageId` (that page is never a tab) and reads **only `rightTabs`** for the split pane. So any `openSplit` on the world route must pass `{ preferSide: 'right' }` — the default rule ("left when `leftTabs` is empty and focus is left") files the tab on a side that route never renders, and the split silently no-ops. It renders `SplitPaneShell.web.tsx` (draggable divider) wrapping two `PagePaneContent` instances when split is active. **Second gotcha — split view mounts two of every editor.** `LoreCanvasEditor` has to listen on `window` for paste, first-keypress-materializes-a-block, and the Ctrl-key shortcuts (those events arrive with nothing in the canvas focused), so both panes receive every one. Instances arbitrate through a module-level claim (`claimGlobalInput` / `ownsGlobalInput`) held by whichever canvas the user last touched or focused. Any new `window`-level handler in a pane-rendered component needs the same guard — without it the unfocused pane steals the event (that's how Ctrl+V pasted into the *other* page). `PagePaneContent` is the extracted page-kind dispatcher that both panes render — it owns edit locks, mention data, save state, and delegates to the specialized view (Location/NPC/Faction/Timeline/PCStub) or the default generic wiki view. Each pane runs its own heartbeat independently. Entry: right-click sidebar page → "Open in split view".
...
- **A WITH CHECK that calls a helper on the column you're clearing can never pass.** `characters: dm can update party members` uses `is_campaign_dm(campaign_id)` for both USING and WITH CHECK, so a DM detaching a character (`campaign_id = null`) is rejected — WITH CHECK runs `is_campaign_dm(null)` against the *new* row. This blocked player removal, which needs two writes: campaign membership is granted by a `campaign_members` row **or** by owning a character whose `campaign_id` points at the campaign (see `is_campaign_member`), so deleting only the membership row leaves the removed player with full read access. Fixed with the `remove_campaign_member` security-definer RPC (migration `20260624000000`) that owns both writes and authorizes DM-or-self. Same lesson as the create-flow RPC: when a policy can't express the transition, move it server-side.
...
- **Realtime silently drops oversized row payloads.** `postgres_changes` truncates the `new`/`old` record when the row exceeds `realtime.max_record_bytes` (default ~1 MB), delivering an empty/partial object (and an `errors: ['Error 413: Payload Too Large']` marker) instead of failing loudly. A handler that blindly merges `payload.new` will then *blank* the columns it carries. This caused the party "Character data unavailable." flicker: characters with large `resources`/`base_stats` JSONB (full spellbook, journal, etc.) load fine via the trimmed RPC, but their UPDATE payload is too big, so `payload.new` arrives with null `base_stats`/`resources` and `PartyMemberCard` flips to the fallback. Fix (in `CampaignPageV2`): guard the merge — if the heavy columns are absent, refetch the trimmed roster (small, always succeeds) instead of clobbering good local state. Always treat a Realtime `new` record as potentially partial.
...
**Primitives** — prefer `@vaultstone/ui` primitives (`Surface`, `Card`, `GradientButton`, `GhostButton`, `Input`, `Chip`, `MetaLabel`, `SectionHeader`, `ScreenHeader`, `Text`, `GlassOverlay`, `Icon`) over raw RN components on new screens. Screen-level reskin (Phase C) is incremental — existing `StyleSheet`-based screens keep rendering with the new palette via legacy aliases. **`Input` auto-grows when `multiline`** (opt out with `autoGrow={false}`, or by setting an explicit `height` in `style` — which is how the AI composer clamps itself). Set `minHeight` in `style` for the starting size.

**Auto-grow gotcha — never feed `contentSize.height` straight back into `height`.** react-native-web measures a multiline field with `scrollHeight`, which never reports *less* than the height already applied to the element. So the naive loop ratchets: every keystroke adds a line and nothing ever shrinks it back, backspace included (that's how the character-sheet Lore fields grew a line per character typed). The sizing lives in one place — **`useAutoGrow` in `@vaultstone/ui`** — which trusts `contentSize` only for the first web measurement (on mount, before any height is applied; that's what sizes a field to its existing text) and re-measures every later change in `onChange`, collapsing the node to `auto` to read its real height before restoring it. `Input`, `LoreTab`'s `ExpandableInput`, and the AI composer all go through it. Pass `paddingY` / `borderY` when the field carries its own padding or border — native's `contentSize` excludes both and web's `scrollHeight` excludes the border, but RN's `height` is border-box.

**Modal backdrop gotcha — a tap-to-dismiss backdrop must not *wrap* a sheet that contains a `TextInput`.** The `<Touchable backdrop onPress={close}><View onStartShouldSetResponder={() => true}>…` shape relies on the sheet claiming the responder to stop the press bubbling, and a web `TextInput` doesn't play along — focusing the search field inside the Combat tab's "Add condition" picker closed the whole modal. Render the backdrop as a **sibling** `<Pressable style={StyleSheet.absoluteFillObject} onPress={close} />` under the sheet instead (see `DetailModal`): separate subtrees, so no press inside the sheet can reach it. A nested inner `Pressable` that has its own `onPress` handler also works (the coin editor in `GearTab`) — it's specifically the bare-`View`-plus-responder form that breaks.
...
1. **Tier 1 — `npm run typecheck`.** Run on every push. Goal is "no net new errors," not "zero errors." The repo has a known baseline of **13 error lines across 3 files** (verified 2026-08-29): `components/character-sheet/CharacterSheet.tsx` (3 — a `Dnd5eStats` null-narrowing case), `packages/store/src/index.ts` (4) and `packages/types/src/database.types.ts` (6) — both duplicate-identifier errors left by the AI-assistant merge. Track the count before and after your changes; if the baseline looks different from this, `git stash` and re-measure on clean master rather than trusting the number written here.
