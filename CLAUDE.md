...
**Primitives** — prefer `@vaultstone/ui` primitives (`Surface`, `Card`, `GradientButton`, `GhostButton`, `Input`, `Chip`, `MetaLabel`, `SectionHeader`, `ScreenHeader`, `Text`, `GlassOverlay`, `Icon`) over raw RN components on new screens. Screen-level reskin (Phase C) is incremental — existing `StyleSheet`-based screens keep rendering with the new palette via legacy aliases. **`Input` auto-grows when `multiline`** (opt out with `autoGrow={false}`, or by setting an explicit `height` in `style` — which is how the AI composer clamps itself). Set `minHeight` in `style` for the starting size. On web the growth measurement collapses the field to `auto` first, because react-native-web measures with `scrollHeight`, which never reports less than the height already applied — measure naively and the box grows but never shrinks.
