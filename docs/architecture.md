...
5. Entries are upserted into the Supabase `imported_content` table under a `homebrew_packs` row owned by the importer. Each import inside a pack is identified by a user-given `source_label` (the "card" inside the pack); same label re-imported refreshes that card, new label adds another card. Re-imports replace via `(pack_id, source_label, entry_key)` so removed source entries don't linger. The top-level Import button on a system page creates a new pack with the first card; an existing pack's detail page appends more cards.
