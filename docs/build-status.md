...
| 8 — Polish & deletion UX | 🟡 | **Done:** Recently Deleted restore UI (list + restore RPCs for pages/sections/maps, `RecentlyDeletedModal`, sidebar "Trash" link), deleted-target mention chips, force-unlock RPC + UI, BEFORE trigger for body_text/body_refs derivation. **Still outstanding:** daily hard-delete cron + Storage reaper, weekly bucket reconciliation, template-upgrade modal, Android editor perf tuning, a11y + keyboard pass. |
| 9 — Quest Tracking | 🟡 | New `quests` section template (`quests.v1.json`) with status/priority/quest_giver/reward/locations/related_npcs fields. `QuestPageView` dedicated page view with status badge, priority pill, quest giver link, LoreCanvasEditor, right sidebar (backlinks, seen in play). `QuestSectionView` with status filter chips (Active/Completed/Failed/On-Hold/All). Quest kind added to relation web constants + structural field edge for quest_giver. Migration `20260514000000` extends template_key + page_kind constraints. |
| 10 — Entity Dossier | 🟡 | `NodeDetailCard` expanded into full dossier panel on the relationship web. On node click: shows structured field properties from template, relationships grouped by source (manual/structural/mention), backlinks (pages mentioning this entity via `getPagesLinkingTo`), timeline appearances (events referencing this entity via `getEventsReferencingPage`). Scrollable body, wider panel (380px). `useNodeDossier` hook encapsulates async data fetching. |
| 11 — Session Prep | 🟡 | Pin pages to a session prep scratchpad viewable on the world home page. `SessionPrepPanel` renders pinned page cards from `structured_fields.__pinned_pages` on the prep page (referenced via `campaigns.next_session_prep_page_id`). "Pin to session prep" action added to `PageContextMenu` (optional `onPinToPrep` callback). `pinPageToPrep` helper function for programmatic pinning. Clear prep button post-session. |
