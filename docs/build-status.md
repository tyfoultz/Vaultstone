...
- [ ] **Phase C — Screen reskin.** ~25 content screens (campaigns, characters, campaign detail tabs, character wizard, session mode, notes). Legacy tokens.ts aliases mean these already render with the Noir palette; visual-system migration is incremental. Screen-by-screen ~0.5–1 day each.
