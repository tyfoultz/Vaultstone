...
### 9. World Builder & Campaign Knowledge Base 🟡 Phase 7d In Progress
...
**Status:** Phase 7c (Search) complete on
`feature/world-builder-phase-7a`. Phases 1–6 shipped to master. Shipped incrementally:
...
| 7d — Sidebar overhaul | 🟡 | **Rail simplification:** removed 5 section icons, keep Home/Map/Timeline/Settings. **Always-visible sections:** sidebar shows all sections simultaneously (no active-section filter). **Persistent collapse:** Zustand + AsyncStorage store for section/page collapse state; chevrons on parent pages; auto-reveal ancestors of active page. **Sub-page creation:** hover "+" on page rows (web), parentPageId threaded through CreatePageModal. **Context menu:** right-click (web) / long-press (native) with Indent, Outdent, Move up/down, Move to section, Rename, Delete — all wired to existing `movePage` RPC with cycle detection. **MoveToSectionModal** for cross-section moves. **Drag-and-drop (web):** react-dnd with Notion-style 3-zone drop targets (before/child/after), visual drop indicators. Native DnD stubbed for future. Pushed on `feature/world-dashboard-cleanup`. |
| 8 — Polish & deletion UX | ⬜ | Recently Deleted restore, daily hard-delete cron + Storage reaper, weekly bucket reconciliation, template-upgrade modal affordance on pages, Android editor perf tuning if needed, a11y + keyboard pass. |
