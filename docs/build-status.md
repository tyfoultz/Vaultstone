...
### 9. World Builder & Campaign Knowledge Base 🟡 Phase 3 Shipped · Phase 4 In Progress
...
| 4 — Visibility, lens, PC stubs, permissions | 🟡 | **4a shipped:** `VisibilityBadge` interactive toggle on page screen + section grid/list cards; owner click flips `visible_to_players` with optimistic store update + rollback on RPC error; non-owners see read-only eye. Still to build: section-level visibility overrides (`force_hidden_from_players`, `default_pages_visible`), RLS helpers (`user_can_view_page` / `user_can_edit_page`), PC-stub lifecycle triggers, `LensDropdown` (`.campaign-switch` handoff chrome) + entry heuristic + mid-session switch banner, orphan banner, Player View preview toggle. `world_page_permissions` + `ShareModal` with direct / inherited source chips + cascade toggle. |
