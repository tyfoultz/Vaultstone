...
| 5.2 — Roster management | ✅ Done | Start Session and Manage members both list every character a player owns under the campaign (from `get_characters_for_campaign_trimmed`, not the pinned-character roster join) and let the DM pick which one is active — previously nothing in the app could repoint `campaign_members.character_id` once set, so a player's second character was unreachable. Attendance stays per-user; the character pick is applied before the session opens. Player removal moved to the `remove_campaign_member` security-definer RPC (migration `20260624000000`): membership alone wasn't enough, since `is_campaign_member` also grants access via an attached character, so a removed player kept read access and kept rendering in the roster. |
...
| 15 — Canvas tables + split-view targeting | ✅ | **Lore canvas tables:** inserted tables now carry an empty paragraph on each open side (`ensureTableSpacers`), so text can be typed above and below them — previously a table landed as the block's first child and the browser refused to place a caret before it. The normalizer also runs on block mount, so tables saved before this fix become editable-around without a migration. Row/column structure editing added: right-click any cell for insert row above/below, insert column left/right, delete row/column/table (`applyTableOp`, DOM-level ops re-read into `htmlRef` afterwards); Tab out of the last cell appends a row. **Split view:** `openSplit`'s default landing side is 'left' when `leftTabs` is empty and focus is left — but the standalone world route renders its left pane from the URL and only reads `rightTabs`, so the first "Open in split view" of every page load filed the tab on a side nothing renders and appeared to do nothing. `PageContextMenu` now pins `preferSide: 'right'` outside the embedded campaign shell; the route's own split-navigate and swap-panes callbacks do the same. **Split-view input ownership:** `LoreCanvasEditor`'s window-level paste / first-keypress / Ctrl-shortcut handlers fired in *both* panes, so Ctrl+V while typing in one page was `preventDefault`ed by the other pane and pasted into a new block there instead. Instances now arbitrate through a module-level claim (`claimGlobalInput` / `ownsGlobalInput`) held by the last-touched canvas; claiming also clears the losing pane's pending-click caret. **Homebrew form fields:** the `Input` primitive now auto-grows when `multiline`, so Properties/Description (and every other prose field across the Spell/Creature/Item/Feat/Class/Species/Background/Condition/Deity/Subclass forms) expand with their content instead of scrolling inside a short fixed box. Opt out with `autoGrow={false}` or an explicit `height`. **Image copy:** Ctrl+C over a selected canvas image silently did nothing — `navigator.clipboard.write` needs the caller's user activation, and activation doesn't survive the `await`s that fetch and PNG-transcode the image, so Chrome rejected the write with NotAllowedError into an empty `catch`. Fixed by passing the blob as a *Promise* inside `ClipboardItem` so `write()` stays inside the activation (`components/world/copyImageToClipboard.ts`, shared by both editors). Both image right-click menus gained a **Copy image** entry — those menus `preventDefault` the browser's own, so without it there was no way to get a picture out of the page. The lore canvas menu now also opens on untagged legacy data-URL images (copy works; caption/pin stay disabled since they need a `world_images` row). **Free-text page refs:** a `page_ref` structured field (faction Leader/HQ, location Parent location, NPC Faction, lore Era) could only hold a link to an existing page — there was no way to name a leader who has no page yet. The field now holds *either* a page id or a plain name, told apart by UUID shape (`components/world/pageRefValue.ts`). No schema or migration: every existing reader already did `pages.find(p => p.id === value)` and skipped a miss, which is the right behaviour for free text — an unlinked name draws no relation-web edge and joins no derived list. Shape-matching also keeps a *deleted* target rendering as blank rather than as a raw UUID. Both pickers (the faction header chips and the shared `PageRefField`) gained a “Use “X”” row plus Enter-to-commit; free text renders unaccented so a clickable page link stays visually distinct. Fixes a latent crash too: a free-text HQ used to reach `getPinsForPage()` as a non-UUID. |
| 16 — Playtest: lore auto-grow, condition picker, coin transactions | ✅ | **Lore field runaway growth:** every keystroke in a character-sheet Lore field (Clothing & Accessories, and every Personality block) added a line to the box — backspace too — and it only reset by leaving the page. `ExpandableInput` fed `contentSize.height + 20` straight back into `height`, and react-native-web derives `contentSize` from `scrollHeight`, which never reports less than the height already applied; the loop could only ratchet upward. Sizing moved into a shared **`useAutoGrow`** hook in `@vaultstone/ui` that trusts `contentSize` only for the first web measurement and re-measures later changes in `onChange` after collapsing the node to `auto`. `Input` (which already carried a private copy of the correction) and the AI composer now go through the same hook — the composer additionally resets its height on send, so it no longer stays tall after a long message. **"Add condition" picker dismissed on search focus:** clicking into the picker's search field closed the modal. The backdrop was a `TouchableOpacity` *wrapped around* the sheet, relying on `onStartShouldSetResponder` for the sheet to swallow the press — which a web `TextInput` defeats. Backdrop is now a sibling absolute-fill `Pressable` beneath the sheet (the `DetailModal` shape), so presses inside the sheet have no path to it. **Coin add/remove:** the coin editor was set-value only, so handing out a 50 gp reward meant doing the arithmetic by hand. `CoinEditModal` gained a Set / Add / Subtract segmented control with the current balance and a live result preview; switching modes clears the field since the same number means different things per mode. Subtracting more than the purse holds is **blocked** (Apply disabled, inline "only N available") rather than clamped to zero — coins are never auto-converted across denominations, so the app can't know which higher coin to break. Logic is exported as `resolveCoinEdit` for testing. |
