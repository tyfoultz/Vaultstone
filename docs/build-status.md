...
| Phase 6 — Character Builder Options | 🟡 In Progress | Data layer + enforcement surfaces behind the campaign rules toggles (multiclassing / customize_origin / optional_class_features / feats_at_level_1 / enforce_feat_prerequisites). Phases 1–5 done: structured types, feat prereq transform (14/14 SRD feats parsed), multiclass prereq transform (24/24 classes), species swap rules (5.1 locked, 5.2 swap-everything), `class-feature-variant` kind. Phases 6–9 outstanding: homebrew feat authoring form, Character Builder tab on system detail page, wizard wiring, character-sheet feat picker. See [features/character-builder.md](features/character-builder.md). |
