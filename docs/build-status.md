...
| 2.5 — Initiative rolling + combat start | ✅ Done | Combatant rows store an **init modifier** (not total); dedicated rolling phase. DM can Roll All, roll per-row, or manually enter a d20 value; players can roll for their own PC via `roll_combatant_initiative` RPC (security-definer ownership check). Start Combat locks in `combat_started_at` and sets round 1; Next Turn only enabled after. Sort: total desc → mod desc → PC > NPC → id. Reset Initiative clears rolls and reopens setup. |
