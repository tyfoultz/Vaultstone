...
| 13 — Player visibility | ✅ | `ShareModal` gains "Player visibility" section: "Show to all players" toggle + per-player checkboxes grouped by campaign. Individual grants use `world_page_permissions`. Interactive `VisibilityBadge` now visible in the topbar of every page template (NPC, Faction, Location, Timeline, PCStub, PagePaneContent) — click to toggle `visible_to_players` without opening the Share modal. |
