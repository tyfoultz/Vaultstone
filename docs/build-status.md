...
| 5 — Campaign Notes Hub (Epic 8 in 06-notes.md) | ✅ Done | Dedicated DM-only route `/campaign/[id]/recap`: session picker (live session pinned first), Markdown recap editor with AsyncStorage-backed drafts, DM's own session-notes editor (RLS migration `20260418000000` lets DMs edit their own notes on any session, ever), and read-only participant-notes list rendered as Markdown. Live `SessionNotesPanel`, `SessionHistoryCard` summaries, and per-player notes all use the shared `RichTextEditor` / `RichTextRenderer` Markdown surface. "Insert from player" text-lift intentionally deferred. |
